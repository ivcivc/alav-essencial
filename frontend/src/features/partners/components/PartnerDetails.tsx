 ...
 if (!partner) {
  return (
   <div className="space-y-6">
    ...
    <div className="text-center py-12">
     <p className="">Parceiro não encontrado</p>
    </div>
   </div>
  )
 }
  ...
  <div className="space-y-6">
   ...
   <div className="flex items-center justify-between">
    <div className="flex items-center space-x-4">
     ...
     <div>
      <h1 className="text-2xl font-bold text-muted-foreground">{partner.fullName}</h1>
      ...
      <Card>
       ...
       <CardContent className="space-y-4">
        <div>
         ...
         <p className="text-muted-foreground">{partner.fullName}</p>
        </div>
        <div>
         <label className="text-sm font-medium text-muted-foreground">CPF/CNPJ</label>
         <p className="text-muted-foreground font-mono">{partner.document}</p>
        </div>
        ...
        {partner.street ? (
         <>
          ...
          <div>
           ...
           <p className="text-muted-foreground">{partner.neighborhood || 'Não informado'}</p>
          </div>
          <div>
          ...
          <div>
           ...
           <p className="text-muted-foreground font-mono">{partner.zipCode || 'Não informado'}</p>
          </div>
         </>
        ) : (
         <p className="text-muted-foreground text-center py-4">Endereço não cadastrado</p>
        )}
       </CardContent>
      </Card>
       ...
       <Card>
        ...
        <CardContent className="space-y-4">
         {partner.bank ? (
          <>
           <div>
            <label className="text-sm font-medium text-muted-foreground dark:text-muted-foreground">Banco</label>
            <p className="text-muted-foreground dark:text-muted-foreground">{partner.bank}</p>
           </div>
           <div className="grid grid-cols-2 gap-4">
            <div>
             <label className="text-sm font-medium text-muted-foreground dark:text-muted-foreground">Agência</label>
             <p className="text-muted-foreground dark:text-muted-foreground font-mono">{partner.agency || 'N/A'}</p>
            </div>
            <div>
             <label className="text-sm font-medium text-muted-foreground dark:text-muted-foreground">Conta</label>
             <p className="text-muted-foreground dark:text-muted-foreground font-mono">{partner.account || 'N/A'}</p>
            </div>
           </div>
           {partner.pix && (
            <div>
             <label className="text-sm font-medium text-muted-foreground dark:text-muted-foreground">Chave PIX</label>
             <p className="text-muted-foreground dark:text-muted-foreground font-mono">{partner.pix}</p>
            </div>
           )}
          </>
         ) : (
          <p className="text-muted-foreground text-center py-4">Dados bancários não cadastrados</p>
         )}
        </CardContent>
       </Card>
