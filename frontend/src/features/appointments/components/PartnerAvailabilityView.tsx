...
const SLOT_COLORS: Record<SlotStatus, string> = {
 ...
 unavailable: 'bg-card  border-border '
}

const SLOT_ICONS: Record<SlotStatus, React.ReactNode> = {
 available: <CheckCircle className="w-4 h-4 text-green-600 dark:" />,
 busy: <XCircle className="w-4 h-4 text-red-600 dark:" />,
 blocked: <AlertCircle className="w-4 h-4 text-yellow-600 dark:" />,
 unavailable: <XCircle className="w-4 h-4 text-muted-foreground dark:text-muted-foreground" />
}
   ...
   <Card>
    <CardContent className="p-6">
     <div className="flex items-center justify-center h-64">
      <div className="text-muted-foreground">Carregando disponibilidade dos parceiros...</div>
     </div>
    </CardContent>
   </Card>
  )
   ...
   <CardContent>
    ...
    <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
     {filteredPartners.map(partner => {
      ...
      return (
       <div 
        key={partner.id} 
        className={`bg-card  p-3 rounded-lg cursor-pointer hover:bg-card dark:hover:bg-card transition-colors ${
       ...
       >
        <div className="flex items-center gap-2 mb-1">
         ...
         <div className="font-medium text-sm dark:text-muted-foreground">{formatDoctorName(partner.fullName)}</div>
        </div>
    ...
    <div className="overflow-x-auto">
     <table className="min-w-full border-collapse border border-border  bg-card ">
      ...
      <thead>
       <tr>
        <th className="p-3 text-left font-medium text-sm text-muted-foreground dark:text-muted-foreground bg-card  border border-border  w-[150px]">
        ...
        {filteredPartners.map(partner => (
         <th 
          key={partner.id}
          className="p-3 text-center font-medium text-sm bg-card  border border-border  cursor-pointer hover:bg-card dark:hover:bg-card min-w-[120px]"
          onClick={() => onPartnerClick?.(partner)}
         >
          <div className="flex flex-col items-center gap-1">
           <div className="flex items-center gap-1">
            <Briefcase className="w-3 h-3 text-muted-foreground dark:text-muted-foreground" />
            <div className="font-medium dark:text-muted-foreground">
      ...
      <tbody>
       {HOUR_SLOTS.map(time => (
        <tr key={time}>
         {/* Coluna do horário */}
         <td className="p-3 text-sm text-muted-foreground dark:text-muted-foreground font-medium bg-card  border border-border ">
         ...
         {filteredPartners.map(partner => {
          ...
          return (
           <td 
            key={`${partner.id}-${time}`}
            className={`p-2 border border-border  transition-colors ${SLOT_COLORS[slotStatus]} h-[60px]`}
            onClick={() => {
            ...
            <div className="flex items-center justify-center h-full">
             {slotStatus === 'busy' && appointment ? (
              <div className="text-center w-full">
               <div className="text-xs font-medium truncate leading-tight text-muted-foreground dark:text-muted-foreground">
                {appointment.patient?.fullName}
               </div>
               <div className="text-xs text-muted-foreground dark:text-muted-foreground leading-tight mt-0.5">
              ...
              <div className="text-center">
               ...
               <div className="text-xs leading-tight text-muted-foreground dark:text-muted-foreground">
