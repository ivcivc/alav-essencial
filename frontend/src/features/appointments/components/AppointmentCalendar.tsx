   ...
   <div className="space-y-2">
    {HOUR_SLOTS.map(time => {
     ...
     return (
      <div 
       key={time} 
       className="grid grid-cols-12 gap-2 min-h-[60px] border-b border-border  py-2 hover:bg-card dark:hover:bg-card/50 cursor-pointer transition-colors"
       onClick={() => onTimeSlotClick?.(currentDate, time)}
      >
       <div className="col-span-2 text-sm text-muted-foreground dark:text-muted-foreground font-medium flex items-center">
        ...
        {timeAppointments.map(appointment => (
         ...
         >
          <div className="flex items-center justify-between mb-2">
           ...
           <Badge variant="outline" className="text-xs border-border/30 text-primary-foreground/90">
 ...
 const renderWeekView = () => {
  ...
  return (
   <div className="overflow-x-auto">
    <div 
     className="grid grid-cols-8 min-w-[800px] calendar-grid bg-card border border-border rounded-lg"
    >
     {/* Header com horários */}
     <div className="p-3 bg-card  font-medium text-muted-foreground dark:text-muted-foreground border-b border-r border-border ">
      <Clock className="w-4 h-4" />
     </div>
     {weekDays.map(day => (
      <div 
       key={day.toISOString()} 
       className="p-3 text-center border-b border-r border-border cursor-pointer hover:bg-muted transition-colors bg-muted last:"
       onClick={() => onDateClick?.(day)}
      >
       <div className="font-medium text-muted-foreground dark:text-muted-foreground text-sm">
        {format(day, 'EEE', { locale: ptBR })}
       </div>
       <div className={`text-lg font-bold mt-1 ${isSameDay(day, new Date()) ? 'bg-blue-500 dark:bg-blue-600 text-primary-foreground rounded-full w-8 h-8 flex items-center justify-center mx-auto' : 'text-muted-foreground dark:text-muted-foreground'}`}>
        {format(day, 'd')}
       </div>
      </div>
     ))}
     ...
     {HOUR_SLOTS.map(time => (
      <React.Fragment key={time}>
       <div className="p-3 text-sm text-muted-foreground border-r border-b border-border bg-muted font-medium flex items-center">
        {time}
       </div>
       {weekDays.map(day => {
        ...
        return (
         <div 
          ...
          className="p-1 border-r border-b border-border min-h-[50px] hover:bg-muted cursor-pointer transition-colors last:"
  ...
  return (
   ...
   >
    ...
    {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(day => (
     <div key={day} className="p-3 text-center font-medium text-muted-foreground dark:text-muted-foreground border-b border-r border-border  bg-card  last:">
      {day}
     </div>
    ))}
    ...
    {calendarDays.map(day => {
     ...
     return (
      <div 
       key={day.toISOString()}
       className={`p-2 min-h-[100px] border cursor-pointer hover:bg-card ${
     ...
     <div className="flex items-center gap-2">
       ...
       {(['day', 'week', 'month'] as CalendarView[]).map((viewType) => (
        <Button
         ...
         className={`rounded-md px-3 py-1 text-xs font-medium transition-all ${
          view === viewType 
           ? 'bg-card  text-muted-foreground dark:text-muted-foreground shadow-sm' 
      ...
      <div className="flex items-center gap-1">
       <Button
        ...
        className="h-8 w-8 p-0 border-border text-muted-foreground bg-card hover:bg-card  dark:text-muted-foreground  dark:hover:bg-card"
       >
       ...
       <Button
        ...
        className="h-8 px-3 text-xs font-medium border-border text-muted-foreground bg-card hover:bg-card  dark:text-muted-foreground  dark:hover:bg-card"
       >
        Hoje
       </Button>
