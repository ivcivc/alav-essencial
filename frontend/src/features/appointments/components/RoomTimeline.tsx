   ...
   <Card>
    <CardContent className="p-6">
     <div className="flex items-center justify-center h-64">
      <div className="text-muted-foreground">Carregando timeline das salas...</div>
     </div>
    </CardContent>
   </Card>
  )
  ...
  <>
    ...
    <CardContent>
     ...
     {activeRooms.length === 0 ? (
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
       ...
       return (
        <div key={room.id} className="bg-card p-4 rounded-lg border border-border">
         <div className="flex items-center justify-between mb-2">
          <div className="font-medium text-sm text-muted-foreground dark:text-muted-foreground">{room.name}</div>
           ...
           return (
            <div
             ...
             className={`w-2 h-2 rounded-sm ${
              hasAppointment ? 'bg-primary' : 'bg-muted'
             }`}
      ...
      <div className="overflow-x-auto">
       <div className="min-w-fit">
        <div className="bg-card  border-b border-border ">
         <div className="flex">
          <div className="w-20 p-3 border-r border-border  bg-card  font-medium text-sm text-muted-foreground dark:text-muted-foreground sticky left-0 z-10">
           Horário
          </div>
          {activeRooms.map(room => (
           <div 
            key={room.id} 
            className="w-40 p-3 border-r border-border  last: text-center font-medium text-sm flex-shrink-0 bg-card "
           >
            <div className="font-semibold text-muted-foreground dark:text-muted-foreground truncate">{room.name}</div>
            {room.description && (
        ...
        <div className="max-h-96 overflow-y-auto">
         {HOUR_SLOTS.map((time, index) => (
          <div key={time} className={`flex border-b border-border  last:border-b-0 ${index % 2 === 0 ? 'bg-card ' : 'bg-card '}`}>
           {/* Coluna de horário - sticky */}
           <div className="w-20 p-3 border-r border-border  bg-card  flex items-center font-medium text-sm text-muted-foreground dark:text-muted-foreground sticky left-0 z-10">
           ...
           {activeRooms.map(room => {
            ...
            return (
             <div 
              key={`${room.id}-${time}`}
              className={`w-40 p-3 border-r border-border last:border-r-0 min-h-[70px] cursor-pointer transition-all duration-200 flex-shrink-0 ${
               hasAppointment 
                ? 'bg-muted hover:bg-accent border-l-2 border-l-primary' 
                : 'hover:bg-accent hover:border-l-2 hover:border-l-primary'
              }`}
              onClick={(e) => {
              ...
              {timeSlotAppointments.length > 0 ? (
               <div className="space-y-1">
                {timeSlotAppointments.slice(0, 1).map(appointment => (
                 <div key={appointment.id} className="space-y-1">
                  <div className="flex items-center gap-1">
                   ...
                   />
                   <span className="font-medium text-xs truncate dark:text-muted-foreground">
                ...
                {timeSlotAppointments.length > 1 && (
                 <div className="text-xs text-muted-foreground font-medium px-1 rounded">
   ...
   {showMultipleAppointments && multipleAppointmentsPosition && (
    <>
     ...
     {/* Popup */}
     <div 
      className="fixed z-50 bg-card  border border-border  rounded-lg shadow-lg p-2 min-w-64"
      style={{
       left: `${multipleAppointmentsPosition.x - 128}px`, // 128px = metade da largura mínima
       top: `${multipleAppointmentsPosition.y - 10}px`
      }}
     >
      <div className="text-sm font-medium text-muted-foreground dark:text-muted-foreground border-b border-border  pb-2 mb-2">
        ...
        >
         <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
           <div className="font-medium text-sm truncate text-muted-foreground dark:text-muted-foreground">
            {appointment.isEncaixe && '📌 '}
            {appointment.patient?.fullName}
           </div>
      ...
      <div className="border-t border-border  pt-2 mt-2">
       <button
        className="w-full text-left p-2 rounded hover:bg-accent transition-colors text-primary"
        onClick={() => {
         setShowMultipleAppointments(false)
