...
export function FinancialReportTable({ data }: FinancialReportTableProps) {
  ...
  const getTypeBadge = (type: string) => {
    const typeMap = {
      INCOME: { label: 'Receita', variant: 'default' as const, icon: TrendingUpIcon },
      EXPENSE: { label: 'Despesa', variant: 'destructive' as const, icon: TrendingDownIcon },
    }
    
    return typeMap[type as keyof typeof typeMap] || { label: type, variant: 'secondary' as const, icon: DollarSignIcon }
  }
              ...
              .map((flow) => (
                <div key={flow.date} className="space-y-1">
                  ...
                  <div className="text-xs text-muted-foreground flex justify-between">
                    <span className="text-green-600">+{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(flow.income)}</span>
                    <span className="text-red-600">-{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(flow.expenses)}</span>
                  </div>
                </div>
              ))}
          </CardContent>
